...
// thing? It only ever performs the initialize/thread/start handshake — no turn is started, no model is
// called, nothing is written — which makes it cheap enough to run on every change to either side.
//
// It SKIPS rather than fails when `codex` is absent or the handshake does not complete, because a missing
// binary is not a fidelity defect. A skip is reported loudly so it cannot be mistaken for a pass.
...
function handshake(bin, args, params, { timeoutMs = 60000, env = process.env } = {}) {
  return new Promise((resolve) => {
    ...
    const bell = setTimeout(() => done({ error: `no reply in ${timeoutMs}ms: ${err.slice(0, 200)}` }), timeoutMs);
    readline.createInterface({ input: child.stdout }).on("line", (line) => {
      let m; try { m = JSON.parse(line); } catch { return; }
      if (m.id === 1) { send({ jsonrpc: "2.0", method: "initialized" }); send({ jsonrpc: "2.0", id: 2, method: "thread/start", params }); }
      if (m.id === 2) { clearTimeout(bell); done(m.error ? { error: JSON.stringify(m.error) } : { result: m.result }); }
    });
...
for (const c of CASES) {
  ...
  const live = await handshake("codex", ["--strict-config", ...args], params, { env });
  if (live.error) {
    skipped++;
    console.log(`SKIP  ${c.name}\n      live server unavailable: ${live.error.slice(0, 120)}`);
    continue;
  }
  const fake = await handshake(process.execPath, [FAKE, ...args], params, { env: { ...(env ?? process.env), FAKE_SCENARIO: "happy" } });
  if (fake.error) { failed++; console.log(`FAIL  ${c.name}\n      the fixture did not answer: ${fake.error.slice(0, 160)}`); continue; }
...
console.log(skipped ? `\n${skipped} skipped (no live server), ${ran - failed}/${ran} of the rest agree`
  : failed ? `\n${failed}/${CASES.length} diverge` : `\nall ${CASES.length} agree`);
// A skip is not a pass, but it is not a defect either: exit 0 so CI without codex stays usable, and let
// the loud SKIP lines carry the message.
