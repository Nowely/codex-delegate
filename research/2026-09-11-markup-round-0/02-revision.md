...
You say what an agent may touch before it starts, and what it did is checked against the record of the
...
Claude writes each agent's prompt outside your project, so the first run asks your permission to write there.
...
Restart Claude Code or run `/reload-plugins` to apply an update; it keeps your stored answers and run
records either way. Removing the *marketplace* instead uninstalls every plugin that came from it — run
...
- **Reads and runs commands, writes nothing of yours.** It reads anything you can read, and your system
  temp directory is the only thing it may write — enough to run your tests. Several read-only agents
  share a directory at once.
...
one line. The one place the two differ is proof, and that is what [How it works](#how-it-works) is
about.
...
agent read on the way to its answer — under your own Codex sign-in, exactly as if you had run `codex`
yourself.

On disk it uses three places:

- **your system temp directory** — each agent's prompt file, and whatever a read-only agent writes while
  it works.
- **`~/.claude/plugins/data/codex-delegate-nowely`** — answers, run records, write locks, and a Codex
  home private to the plugin that keeps your own Codex configuration and MCP servers out of the turn.
...
Answers and run records age out by count and by age; the private home and the write locks stay.
...
setting that takes away the temp directory a read-only agent needs to run tests — so a review agent
launched through it reviews by reading and reports a confident verdict having run nothing. Nothing fails:
the declined run still exits 0.
...
modes lose the plugin's prefix here: `/seat`, not `/codex-delegate:seat`. Update the clone with
`git pull`.
